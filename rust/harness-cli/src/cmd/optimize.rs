use anyhow::Result;
use clap::Args;
use harness_core::storage::{self, CreateRule, FeedbackFilters, Lifecycle};
...
impl OptimizeCmd {
    pub async fn run(self) -> Result<()> {
        ...
        let since = chrono::Utc::now() - chrono::Duration::days(self.days);
        ...
        for rule in &rules {
            ...
            if fp_rate > 0.4 && observations >= 10 {
                ...
                println!("    Run: synodic lifecycle deprecate {}", rule.id);
                flagged += 1;
        ...
        if !uncovered.is_empty() {
            ...
            for cat in &uncovered {
                let relevant_events = ci_failures
                    ...
                    .filter(|e| {
                        // Heuristic: check if any example patterns appear in tool input
                ...
                if relevant_events >= 3 && !self.dry_run {
                    ...
                    if store.get_rule(&rule_id).await?.is_none() {
                        store
                            ...
                            .await?;
                        println!(
                            "    -> Created candidate rule '{}' (run: synodic lifecycle promote {})",
                            rule_id, rule_id
                        );
                    }
                }
